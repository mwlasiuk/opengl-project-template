...
static const std::string kDefaultShader_ColorPoint_Vertex = R"(
    ...
    layout(location = 0) uniform mat4 uniform_Model = mat4(1.0f);
    ...
    void main()
    {
        ...
        gl_Position = system_transform_uniforms.preview_view_projection * uniform_Model * vec4(attribute_Position, 1.0f);
    }
)";
...
static const std::string kDefaultShader_Grid_Fragment = R"(
    ...
    layout(location = 0) uniform float uniform_GridSpacing = 1.0f;
    layout(location = 1) uniform float uniform_GridMinorPerMajor = 10.0f;
    ...
    void main()
    {
        ...
        const float minor_spacing = uniform_GridSpacing / uniform_GridMinorPerMajor;
...
namespace Core
{
    namespace Default
    {
        const char* ColorPointVertexCode()
        {
            return kDefaultShader_ColorPoint_Vertex.c_str();
        }

        size_t ColorPointVertexSize()
        {
            return kDefaultShader_ColorPoint_Vertex.size();
        }

        const char* ColorPointFragmentCode()
        {
            return kDefaultShader_ColorPoint_Fragment.c_str();
        }

        size_t ColorPointFragmentSize()
        {
            return kDefaultShader_ColorPoint_Fragment.size();
        }

        // ---

        const char* PointVertexCode()
        {
            return kDefaultShader_Point_Vertex.c_str();
        }

        size_t PointVertexSize()
        {
            return kDefaultShader_Point_Vertex.size();
        }

        const char* PointFragmentCode()
        {
            return kDefaultShader_Point_Fragment.c_str();
        }

        size_t PointFragmentSize()
        {
            return kDefaultShader_Point_Fragment.size();
        }

        // ---

        const char* GridVertexCode()
        {
            return kDefaultShader_Grid_Vertex.c_str();
        }

        size_t GridVertexSize()
        {
            return kDefaultShader_Grid_Vertex.size();
        }

        const char* GridFragmentCode()
        {
            return kDefaultShader_Grid_Fragment.c_str();
        }

        size_t GridFragmentSize()
        {
            return kDefaultShader_Grid_Fragment.size();
        }
    }
}
